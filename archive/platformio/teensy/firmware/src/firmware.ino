...
CytronMD motor1_controller(PWM_DIR, MOTOR1_PWM, MOTOR1_DIR);
CytronMD motor2_controller(PWM_DIR, MOTOR2_PWM, MOTOR2_DIR); 
CytronMD motor3_controller(PWM_DIR, MOTOR3_PWM, MOTOR3_DIR);
CytronMD motor4_controller(PWM_DIR, MOTOR4_PWM, MOTOR4_DIR);
...
void setup()
{
	//Permet de controler le relay sur le pin 0 pour le lidar
    pinMode(0, OUTPUT);
    digitalWrite(0, HIGH);
    ...
    {
        nh.spinOnce();
        nh.loginfo("LINOBASE NOT CONNECTED\n\r");
    }
...
void moveBase()
{
    ...
    motor1_controller.setSpeed(motor1_pid.compute(req_rpm.motor1, (current_rpm1)));
    motor2_controller.setSpeed(motor2_pid.compute(req_rpm.motor2, (current_rpm2)));
    motor3_controller.setSpeed(motor3_pid.compute(req_rpm.motor3, (current_rpm3)));  
    motor4_controller.setSpeed(motor4_pid.compute(req_rpm.motor4, (current_rpm4)));    

	
//	motor1_controller.setSpeed(0);
//	motor2_controller.setSpeed(35);
//	motor3_controller.setSpeed(0);
//	motor4_controller.setSpeed(0);
/*	char buffer[50];
	sprintf(buffer, "PID M1 : %lf, currRPM : %d, reqRPM : %d", motor1_pid.compute(req_rpm.motor1, (current_rpm1)), (current_rpm1), req_rpm.motor1);
	nh.loginfo(buffer);
	sprintf(buffer, "PID M2 : %lf, currRPM : %d, reqRPM : %d", motor2_pid.compute(req_rpm.motor2, (current_rpm2)), (current_rpm2), req_rpm.motor2);
	nh.loginfo(buffer);
	sprintf(buffer, "PID M3 : %lf, currRPM : %d, reqRPM : %d", motor3_pid.compute(req_rpm.motor3, (current_rpm3)), (current_rpm3), req_rpm.motor3);
	nh.loginfo(buffer);
	sprintf(buffer, "PID M4 : %lf, currRPM : %d, reqRPM : %d", motor4_pid.compute(req_rpm.motor4, (current_rpm4)), (current_rpm4), req_rpm.motor4);
	nh.loginfo(buffer);
*/
...
void printDebug()
{
    ...
    sprintf (buffer, "\r\nEncoder FrontLeft  : %ld", motor1_encoder.read());
    nh.loginfo(buffer);
